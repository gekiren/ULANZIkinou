...
## 3. バックグラウンドタスク履歴残骸の自動防止・完全同期化ルール
- **コマンド実行の同期完了の徹底**: `run_command` ツールで PowerShell や Git、ファイル操作コマンドを実行する際は、原則として `WaitMsBeforeAsync: 10000` (最大10秒) を指定し、バックグラウンドタスク化させず同一コンテキスト内で同期完結させてください。
- **タスク終了時の自律クリーンアップ**: 一連の作業（ビルド・デプロイ・Gitマージ等）が完了した際、AI側で必ず `manage_task(Action: 'list')` を自律実行し、バックグラウンドに残存している一時タスクが存在する場合は直ちに `manage_task(Action: 'kill')` で完全消去・クリーンアップを行ってから完了報告を行わなければなりません。
